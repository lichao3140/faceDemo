...
import com.arcsoft.arcfacedemo.model.FaceFeatureModel;
...
public class FaceServer {
    ...
    private static List<FaceFeatureModel> faceFeatureModelList;
    ...
    public void unInit() {
        synchronized (this) {
            if (faceFeatureModelList != null) {
                faceFeatureModelList.clear();
                faceFeatureModelList = null;
            }
            if (faceEngine != null) {
                faceEngine.unInit();
                faceEngine = null;
    ...
    private void initFaceList(Context context) {
        synchronized (this) {
            ...
            faceFeatureModelList = new ArrayList<>();
            for (File featureFile : featureFiles) {
                try {
                    ...
                    fis.read(feature);
                    fis.close();
                    faceFeatureModelList.add(new FaceFeatureModel(feature, featureFile.getName()));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        faceFeatureModelList.add(new FaceFeatureModel(faceFeature.getFeatureData(), userName));
                        return true;
                    }
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        if (faceEngine == null || isProcessing || faceFeature == null || faceFeatureModelList == null || faceFeatureModelList.size() == 0) {
            return null;
        }
        ...
        isProcessing = true;
        for (int i = 0; i < faceFeatureModelList.size(); i++) {
            tempFaceFeature.setFeatureData(faceFeatureModelList.get(i).getFeatureData());
        ...
        if (maxSimilarIndex != -1) {
            return new CompareResult(faceFeatureModelList.get(maxSimilarIndex).getName(), maxSimilar);
        }
        return null;
    }
